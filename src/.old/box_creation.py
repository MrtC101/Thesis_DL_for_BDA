...
def find_corner_in_image(target_path: os.path):
    """
        ...
        que contienetodos los polígonos en la imagen.
...
def json2boxes(geo_poly_list, dmg_class_list, target_path):
    box_df = pd.DataFrame(
        columns=["x1", "y1", "x2", "y2", "type", "label", "uid"])
    row = []
    # Encuentra las coordenadas mercator de la esquina inferior
    # y superior de un rectángulo que contiene los polígonos.
    ...
    box_df = box_df.astype(
        {'x1': np.uint16, 'y1': np.uint16, 'x2': np.uint16, 'y2': np.uint16})
...
def create_boxes_dataframe(json_path: os.path, target_path: os.path):
    ...
    with open(json_path, 'r') as j:
        ...
        dmg_class_list = [building['properties']
...
def create_bounding_boxes(zones_list: dict[DisasterZoneFolder]):
    zone: DisasterZoneFolder
    for i, zone in enumerate(zones_list.values()):
        post_json_path = join(zone.get_folder_path(), zone.get_post_json())
        post_target_path = join(zone.get_folder_path(), zone.get_class_mask())
        post_boxes_df = create_boxes_dataframe(
            post_json_path, post_target_path)

        boxes_path = join(zone.get_folder_path(), zone.get_bbox())
        post_boxes_df.to_csv(boxes_path, index=False)

        print_progress("Bounding boxes created:", i, len(zones_list.values()))

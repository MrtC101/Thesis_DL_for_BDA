...
def postprocess(split_raw_json_path, definitive_folder, label_map_json, save_path):
    ...
    for dis_id, tile_id, tile_dict, pred_mask in predicted_dataset:
        ...
        px_metric = MetricComputer(dmg_labels).compute_eval_metrics(px_conf_mtrx)
        save_metric(px_metric, pred_out, file_prefix="pixel")
        
        # INEFICIENTISIMA
        # obj_conf_mtrx = MatrixComputer.tile_obj_conf_mtrx(tile_dict["bld_mask"], pred_mask, dmg_labels)
        # obj_metric = MetricComputer.compute_metrics(obj_conf_mtrx)
        # MetricManager.save_metrics(obj_metric, save_path, file_prefix="object")
        
        gt_bbs_df = get_bbs_form_json(tile_dict["dmg_json"])
        gt_values = list(gt_bbs_df.value_counts(subset=["label"]).items())
        gt_table = pd.DataFrame([(val[0][0],val[1]) for val in gt_values],columns=["Level","Count"])

        pd_bbs_df = get_bbs_form_mask(pred_mask, dmg_labels)
        pd_values = list(pd_bbs_df.value_counts(subset=["label"]).items())
        pd_table = pd.DataFrame([(val[0][0],val[1]) for val in pd_values],columns=["Level","Count"])

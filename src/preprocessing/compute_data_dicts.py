...
def create_data_dicts(split_json_path,data_dict_folder):
    ...
    assert split_json_path.split(".")[1]=="json", f"{split_json_path} is not a json file."
    ...
    for set in ["train","val"]:
        ...
        for data_i in tqdm(curr_tileset):
            ...
            mean[dis_id][tile_id] = compute_mean_stddev(data_i["pre_img"],data_i["post_img"])
    ...
    count_path = os.path.join(data_dict_folder,"all_tiles_building_count_and_area.json")
    ...
    mean_disaster_path = os.path.join(data_dict_folder,"all_tiles_mean_stdev_by_disaster.json")

...
class ShardDataset(Dataset):
    ...
    This dataset class provides access to data stored in shards, as defined by a
    shard_splits.json file.
    It enables loading and retrieval of images and corresponding masks from shard
    files efficiently.
    """
    ...
    def refer_to_shard(self):
        ...
        sh_i = self.shard_i
        self.shard = {
            "pre_img": memmap(self.paths["pre_img"][sh_i], dtype='float64', mode=mode,
                              shape=shape_rgb, offset=128),
            "post_img": memmap(self.paths["post_img"][sh_i], dtype='float64', mode=mode,
                               shape=shape_rgb, offset=128),
            "bld_mask": memmap(self.paths["bld_mask"][sh_i], dtype='uint8', mode=mode,
                               shape=shape_gray, offset=128),
            "dmg_mask": memmap(self.paths["dmg_mask"][sh_i], dtype='uint8', mode=mode,
                               shape=shape_gray, offset=128),
            "pre_org": memmap(self.paths["pre_org"][sh_i], dtype='uint8', mode=mode,
                              shape=shape_rgb, offset=128),
            "post_org": memmap(self.paths["post_org"][sh_i], dtype='uint8', mode=mode,
                               shape=shape_rgb, offset=128),
        }
    ...
    def save_shard(image_patches, out_path, log, split_name, shard_i):
        for file_id, patch_list in image_patches.items():
            ...
            shard_path = os.path.join(
                out_path, f'{split_name}_{file_id}_{str(shard_i).zfill(3)}.npy')
            np.save(shard_path, shard, allow_pickle=False)
            log.info(
                f'Shape of last added shard {file_id} to {f"{split_name}_shard"} list is\
                      {shard.shape}, dtype is {shard.dtype}.')
        del shard

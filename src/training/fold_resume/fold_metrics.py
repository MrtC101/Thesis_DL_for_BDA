...
def compute_config_resume(config_path: FilePath) -> dict:
    best_fold = {"fold_num": -1, "epoch": -1, "val_loss": 0.0}

    for file_name in config_path.get_folder_names():
        fold_folder = config_path.join(file_name)
        metric_dir = fold_folder.join("metrics")

        loss_df = pd.read_csv(metric_dir.join("loss.csv"), index_col="epoch")
        best_epoch = loss_df["val_loss"].idxmax()
        fold_resume = []

        for file in metric_dir.get_files_names():
            if file != "loss.csv" and file.endswith(".csv"):
        ...
        curr_fold = {
            "fold_num": fold_num,
            "epoch": best_epoch,
            "val_loss": loss_df["val_loss"].max()
        }
        best_fold = max(best_fold, curr_fold, key=lambda x: x["val_loss"])
    return best_fold

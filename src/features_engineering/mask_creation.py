...
def geo2pixel(geo_poly_list,target_path):
    """ 
        - Transforma as coordenadas geográficas (lat,long) de los polígonos a pixeles de una imagen de  1024x1024.
        - Crea las bounding boxes para cada polígono de la imagen.
        (Se viola el principio de "Single responsibility" para aprovechar el bucle que itera sobre cada polígono.)
    """
    merc_coords_list = []    
...
def create_mask_target_image(json_path : os.path,target_path: os.path) -> np.array:
    """
    ...
    Returns:
        np.array: Un array de (1024,1024,3) dimensiones que contiene las mascaras para cada polígono.
...
def create_instance_mask(zones_list : dict[DisasterZone],raw_path,proc_path):
    zone : DisasterZone;
    for i,zone in enumerate(zones_list.values()):
        pre_json_path = os.path.join(raw_path+"/labels", zone.get_pre().get_json_file_name());
        pre_target_path = os.path.join(raw_path+"/targets",zone.get_pre().get_target_file_name()); 
        pre_mask = create_mask_target_image(pre_json_path,pre_target_path)
       
        mask_path = os.path.join(proc_path,zone.get_disaster_zone_id()); 
        if not os.path.exists(mask_path):
            os.makedirs(mask_path)
        file = os.path.join(mask_path,f"{zone.get_disaster_zone_id()}_instance_mask.png")
        cv2.imwrite(file,pre_mask)

        end = '\n' if (i+1)==len(zones_list.values()) else '\r'
        print(f"Instance segmentation masks created: {(i+1)}/{len(zones_list.values())}", end=end)
